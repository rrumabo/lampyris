...
OUTPUT_FILE = os.path.join(DATA_DIR, f"uncertainty_aware_mpc_{MPC_MODE}_2025.csv")
...
def load_uncertainty_profile() -> pd.DataFrame:
    ...
    required_columns = {"variable", "hour", "std_error"}
    missing = required_columns.difference(profile.columns)
    if missing:
        missing_str = ", ".join(sorted(missing))
...
def build_reserve_lookup(profile: pd.DataFrame) -> Dict[int, float]:
    reserve_by_hour: Dict[int, float] = {}

    for hour in range(24):
        hour_rows = profile[profile["hour"] == hour]

        load_std = float(hour_rows.loc[hour_rows["variable"] == "load", "std_error"].iloc[0])
        solar_std = float(hour_rows.loc[hour_rows["variable"] == "solar", "std_error"].iloc[0])
        wind_on_std = float(hour_rows.loc[hour_rows["variable"] == "wind_onshore", "std_error"].iloc[0])
        wind_off_std = float(hour_rows.loc[hour_rows["variable"] == "wind_offshore", "std_error"].iloc[0])

        reserve_by_hour[hour] = load_std + solar_std + wind_on_std + wind_off_std

    return reserve_by_hour


def solve_mpc_step(
    risk_adjusted_net_load: np.ndarray,
    soc_now: float,
    config: Dict[str, float],
    n_ref: float,
) -> Dict[str, np.ndarray | float]:
    horizon = risk_adjusted_net_load.shape[0]
    ...
    net_after_battery = risk_adjusted_net_load - (p_dis - p_ch)
...
def run_uncertainty_aware_mpc() -> pd.DataFrame:
    df = load_merged_data()
    profile = load_uncertainty_profile()
    reserve_lookup = build_reserve_lookup(profile)

    df["reserve_margin_mw"] = df["hour"].map(reserve_lookup)
    df["risk_adjusted_net_load_mw"] = df["forecast_net_load_mw"] + df["reserve_margin_mw"]

    n_ref = max(float(df["risk_adjusted_net_load_mw"].abs().quantile(0.95)), 1.0)
    ...
    for idx, row in enumerate(df.itertuples(index=False), start=0):
        ...
        risk_adjusted_net_load = horizon_df["risk_adjusted_net_load_mw"].to_numpy(dtype=float)

        solution = solve_mpc_step(
            risk_adjusted_net_load=risk_adjusted_net_load,
            soc_now=soc,
        ...
        reserve_margin_0: float = float(cast(float, row.reserve_margin_mw))
        risk_adjusted_net_load_0: float = float(cast(float, row.risk_adjusted_net_load_mw))
        ...
        net_load_after_battery_0 = risk_adjusted_net_load_0 - battery_power_0
        ...
        record: Dict[str, float | int | str | pd.Timestamp] = {
            ...
            "reserve_margin_mw": reserve_margin_0,
            "risk_adjusted_net_load_mw": risk_adjusted_net_load_0,
            "battery_charge_mw": p_ch_0,
            "battery_discharge_mw": p_dis_0,
            "battery_power_mw": battery_power_0,
            "soc_before_mwh": soc_before,
            "soc_after_mwh": soc,
            "net_load_after_battery_mw": net_load_after_battery_0,
        }
    ...
    print(f"Saved uncertainty-aware MPC results: {OUTPUT_FILE}")
